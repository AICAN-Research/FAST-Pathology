...
//                modelMetadata = setParameterDialog(modelMetadata, &successFlag);
    ...
    //@TODO. Unfortunate naming, corresponds to the name of the model (model folder name).
    // Should rename once everything is running.
    void ProcessManager::pixelClassifier(std::string process_name)
    {
        // @TODO. Should put a global try/catch so that the software does not crash if inference did!
        std::map<std::string, std::string> modelMetadata = this->_models[process_name]->get_model_metadata();
        ...
        if (! current_image_object->has_renderer(modelMetadata["name"]))
        {
...
//                   m_patchStitcherList[modelMetadata["name"]] = stitcher;
                   ...
                   current_image_object->insert_renderer(modelMetadata["name"], "HeatmapRenderer", someRenderer);
...
//                   }
               ...
               else if ((modelMetadata["problem"] == "segmentation") && (modelMetadata["resolution"] == "high"))
               {
                   ...
                   // @TODO. Will have to choose overall between name and model_name.
...
//                   } while (!data->isLastFrame());
               ...
               {  // TODO: Perhaps use switch() instead of tons of if-statements?
                   ...
                   current_image_object->insert_renderer(modelMetadata["name"], "BoundingBoxRenderer", boxRenderer);
...
//                   m_tumorMap = currMap;
                   ...
                   current_image_object->insert_renderer(modelMetadata["name"], "SegmentationRenderer", someRenderer);
//                   m_rendererTypeList[modelMetadata["name"]] = "SegmentationRenderer";
//                   insertRenderer(modelMetadata["name"], someRenderer);
